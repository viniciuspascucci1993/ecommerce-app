...
addToCartButtons.forEach(button => {
    button.addEventListener('click', function() {
        ...
        const productPrice = parseFloat(this.dataset.price); // Converte o preço para número
        const productImage = this.dataset.image;

        // Cria o HTML para o item no carrinho
        const cartItemHTML = `
            <div class="tf-mini-cart-item">
                ...
                <div class="tf-mini-cart-info">
                    <a class="title link" href="product-detail.html">${productName}</a>
                    <div class="price fw-6">R$ ${productPrice.toFixed(2)}</div>
                    <div class="tf-mini-cart-btns">
                        <div class="wg-quantity small">
                            <span class="btn-quantity minus-btn">-</span>
                            <input type="text" name="number" value="1" class="quantity-input" data-price="${productPrice}">
        ...
        const miniCartItems = document.querySelector('.tf-mini-cart-items');
        miniCartItems.insertAdjacentHTML('beforeend', cartItemHTML);
...
function addToCart(productPrice) {
    subtotal += productPrice; // Adiciona o preço do produto ao subtotal
    updateSubtotal(); // Atualiza o subtotal na interface
}

// Função para atualizar o subtotal na interface
function updateSubtotal() {
    const subtotalElement = document.getElementById('subtotal-value');
    subtotalElement.textContent = `R$ ${subtotal.toFixed(2)} `; // Atualiza o valor formatado
}

// Evento para aumentar a quantidade
document.addEventListener('click', function(event) {
  if (event.target.classList.contains('plus-btn')) {
      const quantityInput = event.target.previousElementSibling; // O input de quantidade
      const currentQuantity = parseInt(quantityInput.value) || 0; // Garante que seja um número
      const productPrice = parseFloat(quantityInput.dataset.price) || 0; // Pega o preço do produto

      // Atualiza a quantidade e o subtotal
      quantityInput.value = currentQuantity + 1; // Aumenta a quantidade
      updateSubtotalWithQuantity(productPrice, 1); // Atualiza o subtotal

  } else if (event.target.classList.contains('minus-btn')) {
      const quantityInput = event.target.nextElementSibling; // O input de quantidade
      const currentQuantity = parseInt(quantityInput.value) || 0; // Garante que seja um número
      const productPrice = parseFloat(quantityInput.dataset.price) || 0; // Pega o preço do produto

      if (currentQuantity > 1) { // Garante que a quantidade não fique abaixo de 1
          quantityInput.value = currentQuantity - 1; // Diminui a quantidade
          updateSubtotalWithQuantity(productPrice, -1); // Atualiza o subtotal
      }
  }
});

// Função para atualizar o subtotal (exemplo, ajuste conforme sua lógica)
function updateSubtotalWithQuantity(price, quantityChange) {
  const subtotalElement = document.getElementById('subtotal'); // Assumindo que você tenha um elemento para o subtotal
  const currentSubtotal = parseFloat(subtotalElement.innerText) || 0;
  subtotalElement.innerText = (currentSubtotal + (price * quantityChange)).toFixed(2); // Atualiza o subtotal
}
// Função para adicionar produto ao carrinho
function addToCart(productPrice) {
    subtotal += productPrice; // Adiciona o preço do produto ao subtotal
...
function updateSubtotal() {
    const subtotalElement = document.getElementById('subtotal-value');
    subtotalElement.textContent = `R$ ${subtotal.toFixed(2).replace('.', ',')}`; // Atualiza o valor formatado
}
...
document.addEventListener('click', function(event) {
    if (event.target.classList.contains('plus-btn')) {
        ...
        if (currentQuantity > 1) { // Garante que a quantidade não fique negativa
            quantityInput.value = currentQuantity - 1; // Diminui a quantidade
            updateSubtotalWithQuantity(productPrice, -1); // Atualiza o subtotal
        }
    }
...
// Função para atualizar o subtotal considerando a quantidade
function updateSubtotalWithQuantity(productPrice, quantityChange) {
    subtotal += productPrice * quantityChange; // Atualiza o subtotal
    updateSubtotal(); // Atualiza a interface
}

// Função para definir a observação e atualizar os atributos do botão ao encontrar o produto
const observer = new MutationObserver(() => {
  const productNameElement = document.querySelector(".product-name");
  const addToCartButton = document.getElementById("addToCartButton");

  if (productNameElement && addToCartButton) {
      const productName = productNameElement.innerText;

      addToCartButton.setAttribute("data-name", productName);
      console.log("Produto adicionado ao carrinho:", productName);
      
      // Para o observador após encontrar o elemento e adicionar o atributo
      observer.disconnect();
  }
});

// Inicia o observador no body para monitorar as mudanças no DOM
observer.observe(document.body, { childList: true, subtree: true });

function searchRedirect() {
  // Captura o termo de pesquisa inserido
  const searchTerm = document.getElementById('searchInput').value.toLowerCase();

  // Define os termos e URLs de destino
  const searchMap = {
      "rasteiras": "file:///C:/Users/Vinicius-PC/Documents/Template_Site_Calcados/ecomus-package/ecomus/shop-collection-sub.html",
      "tenis-masculinos": "/pagina-bolsas",
      "tenios-femininos": "/pagina-acessorios",
      "sandalias": "/sandalias",
      "bolsas": "/bolsas"
      // Adicione mais termos e URLs conforme necessário
  };

  // Verifica se o termo corresponde a alguma URL e redireciona
  if (searchMap[searchTerm]) {
      window.location.href = searchMap[searchTerm];
  } else {
      // Se o termo não for encontrado, redireciona para uma página de resultados ou exibe uma mensagem
      window.location.href = "/pagina-de-resultados?query=" + encodeURIComponent(searchTerm);
  }
  ...
  return false;
    ...
    function addToWishlist() {
      // Seleciona o elemento com a classe 'count-box'
      const countBox = document.querySelector('.count-box');
      
      // Converte o conteúdo para número, incrementa e atualiza o valor
      let count = parseInt(countBox.textContent, 10); // Converte para número inteiro
      count = isNaN(count) ? 0 : count + 1; // Incrementa ou inicializa com 0
      countBox.textContent = count; // Atualiza o valor no HTML
  }
